...
<div class="content-wrapper" style="min-height: 946px;">  
                                 ...
                                 <div class="form-group"><br>
                                    ...
                                    <span class="text-danger"><?php echo form_error('code'); ?></span>
                                        ...
                                        <div id="topicsContainer">
                                            <!-- First topic input field with Add and Remove buttons -->
                                            <div class="input-group mb-2 topic-input">
                                                <input name="topics[]" placeholder="Enter a topic" type="text" class="form-control" value="<?php echo set_value('topic'); ?>" />
    ...
    <?php
        $items = [
            ...
            $subject['lab'],
            $subject['clinical'],
            $subject['cocurricular'],
            $subject['lab']
        ];
        
        $result = array_filter($items); // Remove empty elements
        
        echo implode(", ", $result); // Join non-empty elements with a comma
    ?>
</td>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        topicsContainer.addEventListener('click', function (e) {
            if (e.target.classList.contains('add-topic')) {
                ...
                newTopicDiv.classList.add('input-group', 'mb-2', 'topic-input');
                newTopicDiv.innerHTML = `
                    <input name="topics[]" placeholder="Enter a topic" type="text" class="form-control" />
                    <button type="button" class="btn btn-success btn-sm add-topic ms-2">Add</button>
                    <button type="button" class="btn btn-danger btn-sm remove-topic ms-2">Remove</button>
                `;
                topicsContainer.appendChild(newTopicDiv);
            }
        });

        // Remove a topic input field
        topicsContainer.addEventListener('click', function (e) {
            if (e.target.classList.contains('remove-topic')) {
                const topicDiv = e.target.parentElement;
                if (topicsContainer.children.length > 1) {
                    topicDiv.remove(); // Remove only if there's more than one input field
                } else {
                    alert('At least one topic input must remain.');
                }
            }
        });
    });
