...
function getLedgerName($id)
{
	$CI =& get_instance();
	$CI->load->database();
	$ledgername = $CI->db->select('ledger')->where('id', $id)->get('ledgers')->row()->ledger;
...
function getsubjectcolor($id)
{
	$CI = &get_instance();
	$CI->load->database();
	$name = $CI->db->select('subjects.color')->from('subjects')->join('teacher_subjects', 'teacher_subjects.subject_id=subjects.id')->where('teacher_subjects.id', $id)->get()->row();
	return $name->color;
}
...
function gettopicname($id)
{
	$CI =& get_instance();
	$CI->load->database();
	$name = $CI->db->select('subject_topics.topic')->from('subject_topics')->where('subject_topics.id', $id)->get()->row();
	return $name->topic;
}
...
function get_total_subject_attendence($id, $class_id, $section_id, $subject_id, $date)
{
	$CI =& get_instance();
	$CI->load->database();
	...
	$wherearray = [
		...
		'subject_id' => $subject_id,
		'student_period_attendance.student_id' => $id,
		'type' => 0,
	...
	$total = $CI->db->select('*')
		...
		->join('students', 'students.id = student_period_attendance.student_id')
		->where($wherearray)
...
function get_total_practical_attendence($id, $class_id, $section_id, $subject_id, $date)
{
	...
	$total = $CI->db->select('*')
		...
		->join('student_session', 'student_session.id = clinical_attendance.student_session_id')
		->join('students', 'students.id = student_session.student_id')
		->where($wherearray)
		->where('date <=', $end) // Use parameterized query for date
		->get()
		->result_array();
...
function get_monthly_student_attendance_cm_practical($id, $class_id, $section_id, $subject_id, $date)
{
	...
	$pm = $CI->db->select('*')
		->from('clinical_attendance')->join('student_session', 'student_session.id = clinical_attendance.student_session_id')
		->where($wherearray)
		->where("date BETWEEN '$start' AND '$end'")->get()->result_array();
...
function get_monthly_student_attendance_total_practical($id, $class_id, $section_id, $subject_id, $date)
{
	$CI =& get_instance();
	$CI->load->database();
...
function convertTimeTo24HR($time)
{
	if ($time != '') {
		...
		$twentyFourHourTime = date("H:i", strtotime($time));
