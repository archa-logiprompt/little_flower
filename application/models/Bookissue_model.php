...
// var_dump($data);exit;
    ...
    {
        $this->db->select('book_issues.id,book_issues.return_date,book_issues.issue_date,book_issues.is_returned,booklist.title,booklist.barcode,booklist.book_no,booklist.author')->from('book_issues');
        $this->db->join('booklist', 'booklist.id = book_issues.book_id', 'left');
        if ($member_id != null) {
