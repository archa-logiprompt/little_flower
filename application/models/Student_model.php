...
class Student_model extends CI_Model
{
    ...
    public function getleavedetails($student_id = null)
    {
        $this->db->select('student_leaves.*,students.firstname,students.lastname')->join('students', "students.id=student_leaves.student_id");
        if ($student_id) {
            $this->db->where('student_id', $student_id);
        }

        $res = $this->db->get('student_leaves')->result_array();
    ...
    public function searchByClassSection($class_id = null, $section_id = null, $subject_id = null)
    {
        $admin = $this->session->userdata('admin');
        $centre_id = $admin['centre_id'];
        $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id as student_id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state , classes.awarded_by, students.date_of_completion, students.city , students.pincode ,     students.religion, students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.kuhs_reg_no,students.nationality,students.date_completion, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.date_of_completion,students.duration,students.date_of_result,students.created_at ,students.updated_at,students.father_name,students.rte,students.gender,students.year,students.email,students.virtual_acc_no')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
    ...
    public function searchByClassSectionCategoryGenderRte(
        ...
        $admin = $this->session->userdata('admin');
        $centre_id = $admin['centre_id'];
        $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,   students.date_of_completion,  students.dob ,students.current_address,    students.permanent_address,students.category_id, categories.category,   students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.rte,students.gender')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
    ...
    public function viewassign_ward($class_id, $section_id)
    {
        ...
        $this->db->select('assign_ward.id as assign_ward_id,assign_ward.class_id,assign_ward.section_id,assign_ward.group_id,assign_ward.ward_id,assign_ward.datefrom,assign_ward.dateto,clinical_groupname.group_name,sections.section,classes.class,warddetail.wardname,warddetail.aliasname,warddetail.deptnames,clinical_department.deptname,subjects.name as subject_name')->from('assign_ward');
        ...
        $this->db->join('teacher_subjects', 'teacher_subjects.id=assign_ward.subject_id');
        ...
        if (!empty($res)) {
            ...
            foreach ($result as $key => $val) {
                ...
                $a->subject_name = $val['subject_name'];
    ...
    function conducted($teacher_subject_id)
    {
        ...
        foreach ($final as $key => $hr) {
            ...
            $this->db->where('subject_id', $teacher_subject_id);
    ...
    {
        if ($data['id']) {
            $this->db->where('id', $data['id'])->update('clinical_location', $data);
        } else {
            $this->db->insert('clinical_location', $data);
        }
    }
    ...
    public function getLocation($id = null)
    {
        $this->db->select();
        if ($id) {
            $result = $this->db->where('id', $id)->get('clinical_location')->row();
        } else {

            $result = $this->db->get('clinical_location')->result_array();
        }

        return $result;
    }

    public function clinical_locations_delete($id)
    {
        $this->db->where('id', $id)->delete('clinical_location');
    }

    public function getClassAndSection($id)
    {
        $result = $this->db->select('class_id, section_id')
            ->where('student_id', $id)
            ->order_by('id', 'DESC')
            ->limit(1)
            ->get('student_session')
    ...
    public function getQuestionBank($classSection)
    {

        $result = $this->db->where('cls_sec_id', $classSection)->or_where('cls_sec_id', 0)->get('contents')->result_array();
        return $result;
    }

    public function deleteleave($id)
    {
        $this->db->where('id', $id)->delete('student_leaves');
    }

    public function updateLeaveStatus($id, $status)
    {
        $this->db->where('id', $id)->update('student_leaves', ['status' => $status]);
    }
}
