...
class Leavetypes_model extends CI_model {
    ...
    public function getLeaveType($centre_id) {

        // $query = $this->db->get('leave_types')->where('centre_id',$centre_id);
        $query=$this->db->select('*')->from('leave_types')->where('centre_id',$centre_id)->get();
        // echo $this->db->last_query();exit;
