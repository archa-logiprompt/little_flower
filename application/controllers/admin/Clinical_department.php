...
class Clinical_department extends Admin_Controller
{
    ...
    function clinical_attendance()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $subject_id = $this->db->select('assign_ward.subject_id')->where('group_id', $groupid)->get('assign_ward')->row()->subject_id;
    ...
    function clinical_attendance_report()
    {
        if (!$this->rbac->hasPrivilege('clinical_attendance', 'can_view')) {
            access_denied();
        }
        ...
        $this->session->set_userdata('top_menu', 'clinical_rotation');
        $this->session->set_userdata('sub_menu', 'clinical_rotation/clinical_attendance_report');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['subjectName'] = $this->db->select('name')->join('subjects', 'subjects.id=teacher_subjects.subject_id')->where("teacher_subjects.id", $subject_id)->get('teacher_subjects')->row()->name;

            $split_date = explode('/', $this->input->post('date'));
            $date = $split_date[1] . '-' . $split_date[0];
            $data['date'] = $date;


            $resultlist = $this->student_model->searchByClassSection($classid, $sectionid);
            $data['resultlist'] = $resultlist;



            $month = $split_date[0]; // Example: April
            $monthName = date('F', mktime(0, 0, 0, $month, 1));
            $data['monthName'] = $monthName . " " . $split_date[1];
            $year = $split_date[1];
            ...
            for ($day = 1; $day <= $days_in_month; $day++) {
                $fullDates[] = sprintf("%02d", $day);
            }
            $data['fullDates'] = $fullDates;
            $attendance = array();
            foreach ($resultlist as $index => $student) {
                $attendance[$index]['hours'] = $this->clinicalrotation_model->getClinicalAttendanceHours($split_date[1] . '-' . $split_date[0], $student['student_session_id'], $subject_id);
                foreach ($fullDates as $key => $day) {
                    $currentDate = $split_date[1] . '-' . $split_date[0] . '-' . $day;
                    $attendance[$index][$key] = $this->clinicalrotation_model->getClinicalAttendance($currentDate, $student['student_session_id'], $subject_id);
                }
            }
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/clinical_rotation/clinical_attendance/clinical_attendance_report', $data);
        $this->load->view('layout/footer', $data);
    }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            foreach ($student_session_id as $key => $val) {
                ...
                if ($this->input->post('attendencetype' . $val) == 1) {
                    $total_min = 8;
                } else if ($this->input->post('attendencetype' . $val) == 6) {
                    $total_min = 4;
                } else {
                    $total_min = "";
                }
    ...
    function clinical_locations()
    {
        if (!$this->rbac->hasPrivilege('clinical_locations', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'clinical_rotation');
        $this->session->set_userdata('sub_menu', 'clinical_rotation/clinical_department/clinical_locations');
        $data['title'] = 'Add clinical_department';
        $data['location'] = $this->student_model->getLocation();
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->student_model->addLocation($data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left"> Location Name Added Successfully</div>');
            redirect('admin/clinical_department/clinical_locations');
        }

        $this->load->view('layout/header', $data);
        $this->load->view('admin/clinical_rotation/clinical_department/clinical_locations', $data);
        $this->load->view('layout/footer', $data);
    }

    function clinical_locations_edit($id)
    {
        if (!$this->rbac->hasPrivilege('clinical_locations', 'can_view')) {
            access_denied();
        }
        ...
        $data['title'] = 'Add clinical_department';
        $data['location'] = $this->student_model->getLocation();
        $data['value'] = $this->student_model->getLocation($id);
        $data['id'] = $id;
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->student_model->addLocation($data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left"> Location Name Edited Successfully</div>');
            redirect('admin/clinical_department/clinical_locations');
        }

        $this->load->view('layout/header', $data);
        $this->load->view('admin/clinical_rotation/clinical_department/clinical_locations_edit', $data);
        $this->load->view('layout/footer', $data);
    }

    public function clinical_locations_delete($id)
    {
        $this->student_model->clinical_locations_delete($id);
        redirect('admin/clinical_department/clinical_locations');
    }
}
