...
class Student extends Admin_Controller
{
    ...
    function create()
    {
        ...
        $this->form_validation->set_rules(
            'roll_no',
            'Roll No.',
            array('trim', array('check_exists', array($this->student_model, 'valid_student_roll')))
        );
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // TODO when student is assigned to hostel, it should reflect in fee master
            ...
            if (!empty($hostel_room_id)) {
                ...
                $fee_session_group_id = $this->db
                    ->select('fee_session_groups.id')
                    ->join('fee_session_groups', 'fee_session_groups.fee_groups_id=fee_groups.id')
                    ->where('fee_groups.hostel_room_id', $hostel_room_id)->get('fee_groups')->row()->id;
            ...
            // TODO when student is assigned to route, it should reflect in fee master

            if (!empty($vehroute_id)) {
                $fee_session_group_id = $this->db
                    ->select('fee_session_groups.id')
                    ->join('fee_session_groups', 'fee_session_groups.fee_groups_id=fee_groups.id')
                    ->where('fee_groups.vehroute_id', $vehroute_id)->get('fee_groups')->row()->id;
    ...
    function edit($id)
    {
        ...
        $data['knmc']  = $this->db->select('knmc')
            ->where('id', $id)
            ->get('students')
            ->row()->knmc;
        // var_dump($data['knmc']);exit;
        $this->form_validation->set_rules('firstname', 'First Name', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($hostel_room_id)) {
                ...
                $this->studentfeemaster_model->addHostelInFeeMaster($insert_array);
            }
            ...
            if (!empty($vehroute_id)) {
                ...
                $this->studentfeemaster_model->addTransportInFeeMaster($insert_array);
            }
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    $resultlist2 = $this->student_model->searchFullText($search_text,$carray);
                    $data['resultlist'] = $resultlist2;
    ...
    function passedout()
    {
        ...
        $this->session->set_userdata('sub_menu', 'student/passedout');
        $data['title'] = 'Student Search';
        $class = $this->class_model->get();
        $data['classlist'] = $class;
        $userdata = $this->customlib->getUserData();
        ...
        $var = $this->db->select()->get('students')->result_array();
        $data['var'] = $var;
        // var_dump( $var);exit;
        $button = $this->input->post('search');
        if ($this->input->server('REQUEST_METHOD') == "GET") {
    ...
    function getDateofCompletion()
    {

        $id = $this->input->post('studentId');
        // var_Dump($id);exit;
        echo json_encode($this->db->select('date_of_completion')->where('id', $id)->get('students')->row()->date_of_completion);
    }
    function datecompletion()
    {
        $data = array(
        ...
        $this->student_model->adddateofcompletion($data);
        redirect('student/passedout');
    }
    ...
    function cumulative_record()
    {
        ...
        // $data['maxmark'] = $this->student_model->get_markdetails($student_id);
    ...
    public function leave()
    {
        ...
        $leavelist = $this->student_model->getleavedetails();
        $data['leavelist'] = $leavelist;

        $this->load->view('layout/student/header');
        $this->load->view('student/adminleaveapplication', $data);
    ...
    public function updateleave($id, $status)
    {
        ...
        $this->student_model->updateLeaveStatus($id, $status);
